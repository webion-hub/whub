...
export function ProductAttachmentsOutput(props: GeneralProductOutputProps) {
  return (
    <ProductOutput name="attachments" {...props}>
      {(a) => <ProductAttachmentButtonList attachments={a ?? []} />}
    </ProductOutput>
  );
}
...
export function ProductAttachmentButtonList(
  props: ProductAttachmentButtonListProps
) {
  const { t } = useLanguage();
  ...
  if (areNoAttachments()) return null;

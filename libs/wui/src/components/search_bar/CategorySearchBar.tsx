...
export function CategorySearchBar<T, G>(props: CategorySearchBarProps<T, G>) {
  ...
  const [focusAutocomplete, setFocusAutocomplete] =
  ...
  const getColor = () => {
    if (focus) return `${theme.palette.primary.main} !important`;
  ...
  return (
    <Stack
      ...
      sx={{ width: '100%' }}
      divider={
        <Divider orientation="vertical" flexItem sx={{ marginBlock: 0.5 }} />
      }
    >
      <Dropdown
      ...
      <Autocomplete
        ...
        renderOption={(propsOptions, option: T) =>

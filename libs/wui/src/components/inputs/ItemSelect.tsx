...
export function ItemSelect<T>(props: ItemSelect<T>) {
  ...
  const onAdd = (option: T | null) => {
    if (!option || isAlreadyIn(option)) return;
  ...
  return (
    <>
      <Autocomplete
        ...
        onChange={(_, value) => onAdd(value as T | null)}
        onOpen={props.onOpen}
        groupBy={props.groupBy}
        getOptionLabel={props.getOptionLabel}
        renderInput={props.renderInput}
        renderOption={(p, o) =>
          props.renderOption(p, o, isAlreadyIn(o), (o: T) => onAdd(o))
        }
      />
      <MaybeShow show={items.length !== 0}>
        ...
        >
          {items.map((o, i) =>
            props.children(o, i, onRemove, items.length - 1 === i)
          )}
        </List>
      </MaybeShow>
    </>

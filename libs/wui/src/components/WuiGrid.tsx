...
export function WuiGrid(props: WuiGridProps) {
  ...
  const autoWidth = isAColumn ? '100%' : `${100 / numberOfChildren}%`;

  const width = isInAutoWidth ? autoWidth : undefined;

  const children = React.Children.map(
    props.children ?? [],
    (child: ReactElement, index: number) => {
      if (!isSpacingSetted || numberOfChildren === 1) return child;

      const spacing = theme.spacing(props.spacing / 2, '!important');
      const noSpacing = theme.spacing(0, '!important');

      const top = index === 0 && isAColumn ? noSpacing : spacing;

      const bottom =
        index === numberOfChildren - 1 && isAColumn ? noSpacing : spacing;

      const left = index === 0 && !isAColumn ? noSpacing : spacing;
  ...
  if (isSpacingSetted) {
    const { autoWidth, GridItemProps, children: _, ...gridProps } = props;
